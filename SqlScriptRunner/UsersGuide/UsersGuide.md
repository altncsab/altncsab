...
There is an install shield application generated by NSIS in build pipeline. The install application is installing the executable and the users guide in self containing html format. Installation packages are available in Github Actions page for SqlScriptRunner. Download the zip file and extract the SetupSqlScriptRunner_*.exe (the file name ends with the revision). Start the installer.

### Installation steps

1. Welcome screen
2. Accept the license agreement
3. Select/Enter install location
4. Installation completed

Application is reachable from the Start menu or from search bar.

The application is installed under Altncsab/SqlScriptRunner subfolder.

## Starting up the application

There is installer employed but the executable does not signed. That could rase some security concern what may requires actions on antivirus softwares.
...
When the application starting up it is scanning of the home folder of the user and looking for SQL script files (.sql). If any of the subfolder contains such a file it will be opened and all files will be shown and checked. The loading of the structure if asynchronous and showing a message *"Initializing folder structure..."* before it is applying the loaded file structure.
